<?php

return [
    'permissions' => [
        'expenditures.read',
        'expenditures.create',
        'expenditures.update',
        'expenditures.delete',
        'dashboard.read',
        'dashboard.finance_kpis',
        'dashboard.membership_kpis',
        'members.read',
        'members.create',
        'members.update',
        'members.delete',
        'members.export',
        'members.import',
        'givings.read',
        'givings.create',
        'givings.update',
        'givings.delete',
        'givings.export',
        'givings.approve',
        'campaigns.read',
        'campaigns.create',
        'campaigns.update',
        'campaigns.delete',
        'campaigns.export',
        'campaigns.approve',
        'income.read',
        'income.create',
        'income.update',
        'income.delete',
        'pledges.read',
        'pledges.create',
        'pledges.update',
        'pledges.delete',
        'pledges.export',
        'pledges.approve',
        'pledge_payments.read',
        'pledge_payments.create',
        'pledge_payments.update',
        'reports.read',
        'reports.financial.read',
        'reports.membership.read',
        'reports.export',
        'departments.read',
        'departments.create',
        'departments.update',
        'departments.delete',
        'zones.read',
        'zones.create',
        'zones.update',
        'zones.delete',
        'groups.read',
        'groups.create',
        'groups.update',
        'groups.delete',
        'visitors.read',
        'visitors.create',
        'visitors.update',
        'visitors.delete',
        'visitors.convert',
        'children_ministry.read',
        'children_ministry.create',
        'children_ministry.update',
        'children_ministry.delete',
        'children_ministry.export',
        'discipleship.read',
        'discipleship.create',
        'discipleship.update',
        'discipleship.award',
        'follow_up.read',
        'follow_up.create',
        'follow_up.update',
        'follow_up.delete',
        'follow_up.assigned_only',
        'attendance.read',
        'attendance.create',
        'attendance.update',
        'attendance.delete',
        'attendance.reports.read',
        'alerts.read',
        'alerts.run',
        'alerts.update',
        'alerts.delete',
        'member_timeline.read',
        'leaders.read',
        'leaders.create',
        'leaders.update',
        'leaders.delete',
        'communications.read',
        'communications.create',
        'communications.update',
        'communications.delete',
        'communications.send',
        'users.read',
        'users.create',
        'users.update',
        'users.delete',
        'users.assign_roles',
        'roles.read',
        'roles.update',
        'audit_logs.read',
        'settings.manage',
    ],
    'roles' => [
        'system_admin' => [
            'name' => 'System Admin',
            'description' => 'Full access to the church management system.',
            'permissions' => ['*'],
        ],
        'pastor' => [
            'name' => 'Pastor',
            'description' => 'Senior leadership with read-only visibility across people, ministry, and finance.',
            'permissions' => [
                // Core
                'dashboard.read',
                'dashboard.finance_kpis',
                'dashboard.membership_kpis',
                'reports.read',
                'reports.financial.read',
                'reports.membership.read',
                // People — read only
                'members.read',
                'departments.read',
                'zones.read',
                'groups.read',
                'visitors.read',
                'children_ministry.read',
                'discipleship.read',
                'follow_up.read',
                'leaders.read',
                'member_timeline.read',
                // Ministry — read only
                'attendance.read',
                'attendance.reports.read',
                'alerts.read',
                'communications.read',
                // Finance — read only
                'income.read',
                'expenditures.read',
                'givings.read',
                'campaigns.read',
                'pledges.read',
                'pledge_payments.read',
                // Admin visibility
                'audit_logs.read',
            ],
        ],
        'finance_officer' => [
            'name' => 'Finance Officer',
                'description' => 'Full ownership of all financial records — income, giving, expenditures, campaigns, and financial reports.',
            'permissions' => [
                // Core
                'dashboard.read',
                'dashboard.finance_kpis',
                'reports.read',
                'reports.financial.read',
                'reports.export',
                // People — read only (to identify donors/payees)
                'members.read',
                // Finance — full CRUD + approve
                'expenditures.read',
                'expenditures.create',
                'expenditures.update',
                'expenditures.delete',
                'income.read',
                'income.create',
                'income.update',
                'income.delete',
                'givings.read',
                'givings.create',
                'givings.update',
                'givings.delete',
                'givings.export',
                'givings.approve',
                'campaigns.read',
                'campaigns.create',
                'campaigns.update',
                'campaigns.delete',
                'campaigns.export',
                'campaigns.approve',
                'pledges.read',
                'pledges.create',
                'pledges.update',
                'pledges.delete',
                'pledges.export',
                'pledges.approve',
                'pledge_payments.read',
                'pledge_payments.create',
                'pledge_payments.update',
                // Admin visibility
                'audit_logs.read',
            ],
        ],
        'investment_officer' => [
            'name' => 'Investment Officer / Manager',
            'description' => 'Manages income types and income records, with read-only access to members and tithe records.',
            'permissions' => [
                'dashboard.read',
                'dashboard.finance_kpis',
                'members.read',
                'income.read',
                'income.create',
                'income.update',
                'income.delete',
                'givings.read',
            ],
        ],
        'chief_usher' => [
            'name' => 'Chief Usher',
            'description' => 'People management specialist — full access to people-related modules only.',
            'permissions' => [
                // Core (people-focused)
                'dashboard.read',
                'dashboard.membership_kpis',
                // People — full CRUD
                'members.read',
                'members.create',
                'members.update',
                'members.delete',
                'members.export',
                'members.import',
                'departments.read',
                'departments.create',
                'departments.update',
                'departments.delete',
                'zones.read',
                'zones.create',
                'zones.update',
                'zones.delete',
                'groups.read',
                'groups.create',
                'groups.update',
                'groups.delete',
                'leaders.read',
                'leaders.create',
                'leaders.update',
                'leaders.delete',
                'visitors.read',
                'visitors.create',
                'visitors.update',
                'visitors.delete',
                'visitors.convert',
                'children_ministry.read',
                'children_ministry.create',
                'children_ministry.update',
                'children_ministry.delete',
                'children_ministry.export',
                'discipleship.read',
                'discipleship.create',
                'discipleship.update',
                'discipleship.award',
                'follow_up.read',
                'follow_up.create',
                'follow_up.update',
                'follow_up.delete',
                'member_timeline.read',
            ],
        ],
        'zone_leader' => [
            'name' => 'Zone Leader',
            'description' => 'Cell/zone group leadership — attendance and follow-up for their group.',
            'permissions' => [
                // Core
                'dashboard.read',
                'member_timeline.read',
                // People — read only, limited create
                'members.read',
                'zones.read',
                'groups.update',
                'visitors.read',
                'visitors.create',
                'visitors.update',
                'follow_up.read',
                    'groups.read',
                'follow_up.create',
                'follow_up.update',
                'follow_up.assigned_only',
                // Ministry — scoped to their group
                'attendance.read',
                'attendance.create',
                'alerts.read',
            ],
        ],
        'counsellor' => [
            'name' => 'Counsellor',
            'description' => 'Follow-up counsellor — view and complete assigned follow-up tasks only.',
            'permissions' => [
                'dashboard.read',
                'follow_up.read',
                'follow_up.update',
                'follow_up.assigned_only',
            ],
        ],
        'usher' => [
            'name' => 'Usher',
            'description' => 'Door volunteer — QR scan check-in, visitor registration, and attendance recording only.',
            'permissions' => [
                'attendance.create',
                'members.read',
                'visitors.create',
            ],
        ],
    ],
    'navigation' => [
        [
            'label' => 'Dashboard',
            'route' => 'dashboard.index',
            'permission' => 'dashboard.read',
        ],
        [
            'label' => 'Users',
            'route' => 'users.index',
            'permission' => 'users.read',
        ],
        [
            'label' => 'Members',
            'route' => 'members.index',
            'permission' => 'members.read',
        ],
        [
            'label' => 'Departments',
            'route' => 'departments.index',
            'permission' => 'departments.read',
        ],
        [
            'label' => 'Zones',
            'route' => 'zones.index',
            'permission' => 'zones.read',
        ],
        [
            'label' => 'Groups',
            'route' => 'groups.index',
            'permission' => 'groups.read',
        ],
        [
            'label' => 'Visitors',
            'route' => 'visitors.index',
            'permission' => 'visitors.read',
        ],
        [
            'label' => 'Children Ministry',
            'route' => 'children-ministry.index',
            'permission' => 'children_ministry.read',
        ],
        [
            'label' => 'Discipleship',
            'route' => 'discipleship.index',
            'permission' => 'discipleship.read',
        ],
        [
            'label' => 'Follow-Up',
            'route' => 'follow-up.index',
            'permission' => 'follow_up.read',
        ],
        [
            'label' => 'Leaders',
            'route' => 'leaders.index',
            'permission' => 'leaders.read',
        ],
        [
            'label' => 'Attendance',
            'route' => 'attendance.index',
            'permission' => 'attendance.read',
        ],
        [
            'label' => 'Alerts',
            'route' => 'alerts.index',
            'permission' => 'alerts.read',
        ],
        [
            'label' => 'Communications',
            'route' => 'communications.index',
            'permission' => 'communications.read',
        ],
        [
            'label' => 'Reports',
            'route' => 'reports.index',
            'permission' => 'reports.read',
        ],
        [
            'label' => 'Message Operations',
            'route' => 'communications.operations',
            'permission' => 'settings.manage',
        ],
        // --- Phase 1: Core Finance Modules ---
        [
            'label' => 'Expenditures',
            'route' => 'expenditures.index',
            'permission' => 'expenditures.read',
        ],
        [
            'label' => 'Department Income',
            'route' => 'department-income.index',
            'permission' => 'departments.read',
        ],
        [
            'label' => 'Department Expenses',
            'route' => 'department-expenses.index',
            'permission' => 'departments.read',
        ],
        [
            'label' => 'Income Records',
            'route' => 'income.index',
            'permission' => 'income.read',
        ],
        [
            'label' => 'Income Types',
            'route' => 'income-types.index',
            'permission' => 'income.read',
        ],
        // --- Phase 2: Giving & Campaigns ---
        [
            'label' => 'Givings',
            'route' => 'givings.index',
            'permission' => 'givings.read',
        ],
        [
            'label' => 'Campaigns',
            'route' => 'campaigns.index',
            'permission' => 'campaigns.read',
        ],
        [
            'label' => 'Pledges',
            'route' => 'pledges.index',
            'permission' => 'pledges.read',
        ],
        [
            'label' => 'Missed Pledges',
            'route' => 'missed-pledges.index',
            'permission' => 'pledges.read',
        ],
        [
            'label' => 'Pledge Payments',
            'route' => 'pledge-payments.index',
            'permission' => 'pledge_payments.read',
        ],
        [
            'label' => 'Role Permissions',
            'route' => 'roles.index',
            'permission' => 'roles.read',
        ],
        [
            'label' => 'Audit Log',
            'route' => 'audit-logs.index',
            'permission' => 'audit_logs.read',
        ],
    ],
];
