...
<div class="grid gap-6 lg:grid-cols-2">
    <div>
        ...
        <input id="name" name="name" class="form-input mt-2" value="{{ old('name', $zone->name) }}" required>
        @error('name') <p class="mt-2 text-sm text-red-600">{{ $message }}</p> @enderror
    </div>

    <div>
        <label for="leader_id" class="form-label">Zone leader</label>
        <select id="leader_id" name="leader_id" class="form-input mt-2">
            <option value="">Select leader</option>
            @foreach ($leaders as $leader)
                <option value="{{ $leader->id }}" @selected((string) old('leader_id', $zone->leader_id) === (string) $leader->id)>
    ...
    <div class="lg:col-span-2">
        ...
        <textarea id="description" name="description" rows="4" class="form-input mt-2">{{ old('description', $zone->description) }}</textarea>
...
<div class="mt-6 flex flex-wrap gap-3">
    ...
    <a href="{{ route('zones.index') }}" class="btn-secondary">Cancel</a>
</div>
