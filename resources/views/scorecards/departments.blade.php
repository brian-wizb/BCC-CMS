<x-layouts.app title="Department Scorecards">
    <section class="surface-card p-6 overflow-x-auto">
        ...
        <div class="mb-5 flex items-center justify-between gap-3">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <h2 class="text-xl font-semibold text-[var(--color-ink-950)]">Department Scorecards</h2>
                    <p class="text-xs text-slate-500">Participation &amp; volunteer output by department</p>
                </div>
            </div>
        ...
        <table class="min-w-full divide-y divide-[var(--color-surface-200)] text-sm">
            <thead class="bg-[var(--color-surface-50)] text-left text-slate-500">
                <tr>
                    ...
                    <th class="px-4 py-3"><i class="fas fa-tasks mr-1.5 opacity-60"></i>Completed Volunteer Assignments</th>
                </tr>
            </thead>
            <tbody class="divide-y divide-[var(--color-surface-200)] bg-white">
                @forelse ($departments as $department)
                    <tr>
                        ...
                        <td class="px-4 py-4">{{ $department['member_count'] }}</td>
                        <td class="px-4 py-4">{{ $department['completed_assignments'] }}</td>
                    </tr>
                @empty
                    <tr><td colspan="3" class="px-4 py-8 text-center text-slate-400">No department scorecard data yet.</td></tr>
                @endforelse
            </tbody>
        </table>
    </section>
